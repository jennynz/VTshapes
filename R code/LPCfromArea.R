...
"calc.reflection.coef" <- function(areadata, diameter = FALSE) {
  ...
  refl.coef <- vector(mode = "numeric", length = n)
  
  # Area outside lips is assumed to be infinity (areadata[0]  = inf) therefore
  # the first reflection coefficent is set to 1.
  refl.coef[1] <- 1
...
"rc2lpc.II" <- function(reflcoef) {
  # Converts reflection coefficients to linear predictive coding coefficents
  # using a recursive algorithm. This version uses the algorithm in JMH's book
  # p216 modified march 2nd 2007.
  
  # N.B. This function expects that the first reflection coeffcient is 1
  if(reflcoef[1]!=1) {
...
"lpc2spec.III" <- function(filtcoef, n=128) {
  ...
  # Calculates spectrum from LPC coefficients.
  # Could just zero extend and fft, but wanted to evaluate myself
  # Modified 2/3/07
